...
function TrendChart({ values, labels = ['Now', 'Year 1', 'Year 2', 'Year 3', 'Year 4', 'Year 5'] }) {
  ...
  const rangeConfig = { '1D': { count: 2, labels: ['Open', 'Today'] }, '7D': { count: 4, labels: ['Mon', 'Wed', 'Fri', 'Today'] }, '1Y': { count: 5, labels: ['Now', '3M', '6M', '9M', '1Y'] }, '5Y': { count: 6, labels: ['Now', '1Y', '2Y', '3Y', '4Y', '5Y'] }, ALL: { count: 8, labels: ['Start', '1Y', '2Y', '3Y', '4Y', '5Y', '6Y', 'All'] } }[range]
  const selectedValues = Array.from({ length: rangeConfig.count }, (_, index) => values[Math.min(values.length - 1, Math.round(index * (values.length - 1) / Math.max(rangeConfig.count - 1, 1)))])
  const max = Math.max(...selectedValues, 1)
  const points = selectedValues.map((value, index) => `${index * (680 / Math.max(selectedValues.length - 1, 1)) + 10},${118 - (value / max) * 92}`).join(' ')
  return <div className="calculator-chart"><div className="range-tabs" role="tablist" aria-label="Chart time range">{Object.entries({ '1D': '1 day', '7D': '7 days', '1Y': '1 year', '5Y': '5 years', ALL: 'All time' }).map(([key, label]) => <button key={key} className={range === key ? 'selected' : ''} onClick={() => setRange(key)} aria-label={label}>{key === 'ALL' ? 'All' : key}</button>)}</div><svg viewBox="0 0 700 145" role="img" aria-label="Calculator projection graph"><defs><linearGradient id="chartFill" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor="var(--lime)" stopOpacity=".42" /><stop offset="100%" stopColor="var(--lime)" stopOpacity="0" /></linearGradient></defs><line x1="10" y1="118" x2="690" y2="118" className="grid-line" /><polyline points={`10,118 ${points} 690,118`} fill="url(#chartFill)" stroke="none" /><polyline points={points} fill="none" stroke="var(--lime)" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" />{selectedValues.map((value, index) => <circle key={index} cx={index * (680 / Math.max(selectedValues.length - 1, 1)) + 10} cy={118 - (value / max) * 92} r="4" fill="var(--lime)" />)}</svg><div className="chart-labels">{rangeConfig.labels.map(label => <span key={label}>{label}</span>)}</div></div>
}

function ToolPage({ toolKey, onBack }) {
  const [values, setValues] = useState({ amount: 100000, rate: 12, years: 10, monthly: 15000, stepUp: 10, target: 1000000, inflation: 6, loan: 2500000, tenure: 20, fixedCosts: 300000, variableCost: 450, units: 1000, price: 1000, investment: 500000, returnValue: 650000, employees: 5, salary: 60000, benefits: 12, taxRate: 18, revenue: 1000000, cost: 650000, inventory: 250000, receivables: 180000, payables: 120000, fromAmount: 1000, fromCurrency: 'USD', toCurrency: 'INR' })
  const set = (key, value) => setValues(previous => ({ ...previous, [key]: value }))
  const months = values.years * 12
  const monthlyRate = values.rate / 1200
  const sipFuture = Array.from({ length: months }, (_, month) => values.monthly * Math.pow(1 + values.stepUp / 100, Math.floor(month / 12)) * Math.pow(1 + monthlyRate, months - month - 1)).reduce((sum, contribution) => sum + contribution, 0)
...
function GlobalSearch({ navigate }) {
  const [query, setQuery] = useState('')
  const results = query.trim() ? [...pageItems.map(([key, Icon]) => [key, key, Icon]), ...toolPages].filter(([, label]) => label.toLowerCase().includes(query.toLowerCase())).slice(0, 8) : []
  return <div className="global-search"><Search size={15} /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search pages and tools..." aria-label="Search pages and tools" />{results.length > 0 && <div className="search-results">{results.map(([key, label, Icon]) => <button key={key} onClick={() => { navigate(key); setQuery('') }}><Icon size={14} /><span>{label}</span><ChevronRight size={13} /></button>)}</div>}</div>
}

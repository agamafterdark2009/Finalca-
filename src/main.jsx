...
function CurrencyField({ label, value, onChange }) {
  return <label className="calculator-field"><span>{label}</span><div><select value={value} onChange={event => onChange(event.target.value)}><option value="INR">INR · ₹</option><option value="USD">USD · $</option><option value="EUR">EUR · €</option><option value="GBP">GBP · £</option><option value="AED">AED</option><option value="SGD">SGD · S$</option></select></div></label>
}
...
function ToolPage({ toolKey, onBack }) {
  ...
  const resultLabel = isExchange ? `${values.fromCurrency} → ${values.toCurrency} at ${exchangeRate.toFixed(4)}` : toolKey === 'roi' || toolKey === 'margin' ? 'Percentage return' : toolKey === 'breakeven' ? 'Units to break even' : 'Estimated outcome'

...
const initialData = {
  ...
  Loans: [{ id: 5, date: '2024-04-01', source: 'Home loan', category: 'HDFC · 8.4%', amount: 4200000 }, { id: 6, date: '2023-07-01', source: 'Car loan', category: 'Axis · 9.1%', amount: 680000 }],
...
const alerts = [
  ...
  { title: 'SIP contribution due soon', detail: 'Your next ₹25,000 contribution is scheduled this week.', tone: 'purple', page: 'SIPs' },
...
function CfoCharacter({ onOpen }) {
  return <motion.button type="button" className="cfo-character" onClick={onOpen} aria-label="Open AI CFO chat" initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: .35, duration: .6 }} whileHover={{ y: -6, rotate: 1 }} whileTap={{ scale: .98 }}>
    ...
    <div className="character-copy"><p className="eyebrow">Your AI CFO</p><b>Let’s make your money move.</b><small>Ask me for a clear next step.</small></div>
  </motion.button>
}
...
const financeTools = [
  ...
  ['inflation', 'Inflation calculator', 'See what today’s money may cost in the future.', Percent],
  ...
  ['reports', 'Monthly report', 'Prepare a clean summary for your monthly money review.', FileText],
  ['import', 'Data import', 'Bring existing records into the editable ledgers.', Download],
  ['habits', 'Money habits', 'Set a weekly review ritual and keep momentum.', PiggyBank],
]
...
function ToolPage({ toolKey, onBack }) {
  const [values, setValues] = useState({ amount: 100000, rate: 12, years: 10, monthly: 15000, target: 1000000, inflation: 6, loan: 2500000, tenure: 20 })
  ...
  const emi = values.loan * loanRate * Math.pow(1 + loanRate, values.tenure * 12) / (Math.pow(1 + loanRate, values.tenure * 12) - 1)
  ...
  const toolNotes = {
    budget: ['Recommended split', '50% needs · 30% wants · 20% future', 'Next action', 'Enter take-home income and compare every category.'],
    ...
    retirement: ['Planning lens', 'Build a future income target with today’s assumptions', 'Next action', 'Revisit inflation and expected returns yearly.'],
  ...
  const result = isLoan ? emi : isSip ? (toolKey === 'sip' ? sipFuture : compound) : isInflation ? inflated : toolKey === 'retirement' ? values.monthly * 12 * values.years : values.target
  const chartValues = isLoan ? [values.loan, values.loan * .82, values.loan * .62, values.loan * .38, values.loan * .12] : isInflation ? [values.amount, inflated * .35, inflated * .55, inflated * .72, inflated] : [values.amount * .35, values.amount * .55, result * .72, result * .88, result]
  return <main className={`tool-page tool-${toolKey}`}><button className="back-link" onClick={onBack}><ArrowLeft size={15} />Back to overview</button><section className="tool-hero"><ToolLogo Icon={Icon} toolKey={toolKey} /><div><p className="eyebrow">Dedicated finance tool</p><h1>{title}</h1><p>Change the assumptions and instantly see a clear projection for your plan.</p></div></section><section className="calculator-layout"><article className="panel calculator-panel"><div className="panel-header"><div><p className="eyebrow">Inputs</p><h2>Shape your scenario</h2></div><Calculator size={18} className="positive" /></div><div className="calculator-fields">{isLoan ? <><NumberField label="Loan amount" value={values.loan} onChange={value => set('loan', value)} /><NumberField label="Interest rate" value={values.rate} onChange={value => set('rate', value)} suffix="%" /><NumberField label="Tenure" value={values.tenure} onChange={value => set('tenure', value)} suffix="years" /></> : <><NumberField label={isInflation ? 'Today’s amount' : isSip ? 'Monthly contribution' : 'Starting amount'} value={isSip && toolKey === 'sip' ? values.monthly : values.amount} onChange={value => set(isSip && toolKey === 'sip' ? 'monthly' : 'amount', value)} /><NumberField label={isInflation ? 'Inflation rate' : 'Expected return'} value={isInflation ? values.inflation : values.rate} onChange={value => set(isInflation ? 'inflation' : 'rate', value)} suffix="%" /><NumberField label="Time horizon" value={values.years} onChange={value => set('years', value)} suffix="years" />{!isSip && !isInflation && <NumberField label="Target amount" value={values.target} onChange={value => set('target', value)} />}</>}</div></article><article className="panel calculator-result"><p className="eyebrow">Estimated outcome</p><strong>{money(result)}</strong><span>{isLoan ? `Monthly EMI · total interest ${money(Math.max(0, emi * values.tenure * 12 - values.loan))}` : isInflation ? `Future cost in ${values.years} years` : 'Projected value at the end of your plan'}</span><TrendChart values={chartValues} /></article></section>  <section className="calculator-insights"><article className="summary-card"><span>Starting value</span><b>{money(isLoan ? values.loan : values.amount)}</b><small>your baseline</small></article><article className="summary-card"><span>Growth / cost</span><b>{money(isLoan ? Math.max(0, emi * values.tenure * 12 - values.loan) : Math.max(0, result - values.amount))}</b><small>estimated difference</small></article><article className="summary-card"><span>Review cadence</span><b>Monthly</b><small>refresh assumptions regularly</small></article></section><section className="tool-guidance"><article className="panel guidance-card"><p className="eyebrow">{toolNotes[0]}</p><h2>{toolNotes[1]}</h2><div className="guidance-meter"><i /></div></article><article className="panel guidance-card"><p className="eyebrow">{toolNotes[2]}</p><h2>{toolNotes[3]}</h2><button className="outline-button compact" onClick={() => window.print()}><FileText size={14} />Save this plan</button></article></section></main>
}

function FinanceToolkit({ setActive, setChatOpen, setModal }) {
  const [selected, setSelected] = useState(null)
  const open = tool => setSelected(tool)
  const close = () => setSelected(null)
  const action = key => { close(); setActive(key) }
  return <section className="panel toolkit-panel"><div className="panel-header"><div><p className="eyebrow">20 finance tools</p><h2>Your personal finance toolkit</h2></div><Sparkles size={18} className="positive" /></div><div className="toolkit-grid">{financeTools.map(([key, title, description, Icon], index) => <motion.button className="tool-card" key={key} onClick={() => open({ key, title, description, Icon })} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * .025 }} whileHover={{ y: -5, scale: 1.02 }} whileTap={{ scale: .98 }}><span className="tool-icon"><Icon size={17} /></span><span><b>{title}</b><small>{description}</small></span><ChevronRight size={14} /></motion.button>)}</div><AnimatePresence>{selected && <Modal title={selected.title} eyebrow="Finance toolkit" onClose={close}><div className="feature-hero"><selected.Icon size={24} /><div><b>{selected.title}</b><p>{selected.description}</p></div></div><p className="modal-copy">Use your saved Finalca data to turn this into a repeatable part of your monthly money routine. Your records stay editable and local to this browser.</p><button className="primary-button modal-action" onClick={() => action(selected.key)}>Open related workspace <ChevronRight size={15} /></button><button className="outline-button modal-action" onClick={() => { close(); setChatOpen(true) }}><Bot size={15} />Ask the AI CFO</button></Modal>}</AnimatePresence></section>
}
